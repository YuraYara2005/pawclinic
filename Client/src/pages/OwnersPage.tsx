...
export default function OwnersPage() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    // Note: React keys match MySQL keys here, so no translation needed.
    const payload = {
      name: form.name,
      phone: form.phone,
    ...
    // 🚨 FIX: Removed rogue quotes here
    const url = editing ? `${import.meta.env.VITE_API_URL}/api/owners/${targetId}` : `${import.meta.env.VITE_API_URL}/api/owners`;
  ...
  const handleDelete = async (id: string | number | undefined) => {
    ...
    setIsDeletingId(id);
    try { 
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/owners/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` }
      });
    ...
    <div className="pb-10 animate-in fade-in duration-500 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">{isLoading ? "Syncing clients..." : `${owners.length} registered owners`}</p>
        </div>
